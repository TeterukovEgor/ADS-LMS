...
public class B_LongDivComSubSeq {
    ...
    int getDivSeqSize(InputStream stream) throws FileNotFoundException {
        ...
        //перебор предыдущих перед i элементов
            for (int j = 0; j < i; j++) {
                // отличие от задачи А в том что проверка через деление, а не больше меньше
                // если результат i % на j будет целым, то цепочка увеличивается
                if (m[i] % m[j] == 0 && l[j] + 1 > l[i]) { 
                    l[i] = l[j] + 1;        
                }
            }
