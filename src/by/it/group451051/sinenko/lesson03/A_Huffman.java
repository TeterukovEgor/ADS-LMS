...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        // для каждого символа создаём LeafNode и добавляем в очередь
        for (Map.Entry<Character, Integer> entry : count.entrySet()) {

             // создаём LeafNode (лист дерева)
             LeafNode leaf = new LeafNode(entry.getValue(), entry.getKey());

             // добавляем в PriorityQueue
        ...
        while (priorityQueue.size() > 1) {
            // poll возвращает и удаляет элемент с наивысшим приоритетом (наименьшей частотой)
            ...
            // возвращаем родителя в очередь для дальнейших объединений
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f = new File(root + "by/it/group451051/sinenko/lesson03/dataHuffman.txt");
