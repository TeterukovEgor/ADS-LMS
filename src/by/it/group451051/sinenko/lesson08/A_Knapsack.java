package by.it.group451051.sinenko.lesson08;
...
public class A_Knapsack {
    ...
    int getMaxWeight(InputStream stream ) {
        ...
        int[] maxWeight = new int[w + 1];  // максимальный вес который можно набрать

        for (int i = 0; i < n; i++) {
            gold[i]=scanner.nextInt();
        }

        // для каждой возможной вместимости проверяем каждый тип слитка 
        for(int i = 1; i <= w; i++) {         
            for (int j = 0; j < n; j++) {
                if (gold[j] <= i) {   // если слиток помещается, то берем то что лучше
