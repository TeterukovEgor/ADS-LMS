...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            count.put(c, count.getOrDefault(c, 0) + 1); // Получаем текущее значение частоты символа, увеличиваем на 1 и сохраняем
        }

        //2. перенесем все символы в приоритетную очередь в виде листьев
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for (Map.Entry<Character, Integer> entry : count.entrySet()) {
            LeafNode leaf = new LeafNode(entry.getValue(), entry.getKey()); // Создаем листовой узел для каждого символа с его частотой
            priorityQueue.add(leaf); // Добавляем лист в приоритетную очередь
        }
        ...
        while (priorityQueue.size() > 1) {
            Node left = priorityQueue.poll(); // Извлекаем узел с минимальной частотой (левый потомок)
            Node right = priorityQueue.poll(); // Извлекаем следующий узел с минимальной частотой (правый потомок)
            InternalNode parent = new InternalNode(left, right); // Создаем внутренний узел с суммой частот потомков
            priorityQueue.add(parent); // Возвращаем родительский узел обратно в очередь
        }

        //4. последний из родителей будет корнем этого дерева
        //это будет последний и единственный элемент оставшийся в очереди priorityQueue.
        Node root = priorityQueue.poll(); // Извлекаем корневой узел дерева
        root.fillCodes(""); // Начинаем заполнение кодов символов с пустой строки

        StringBuilder sb = new StringBuilder();
        // Кодируем исходную строку с помощью полученных кодов
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            sb.append(codes.get(c)); // Добавляем код текущего символа к результату
        }
