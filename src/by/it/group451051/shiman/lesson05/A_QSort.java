...
координаты концов отрезков (время работы одной какой-камеры).
...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        // Сортируем отрезки по началу, а если начала равны - по концу
        Arrays.sort(segments);

        // Для каждой точки подсчитываем, скольким отрезкам она принадлежит
        for (int i = 0; i < m; i++) {
            int point = points[i];
            int count = 0;
            ...
            for (Segment segment : segments) {
                // Если начало отрезка уже больше точки, дальше можно не проверять
                // (т.к. отрезки отсортированы по началу)
