...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        char[] mas_char = new char[s.length()];
        int[] mas_s = new int[s.length()];
        int i2,kol=0;
        boolean razr;
        for (int i=0;i<s.length();i++) {
            razr=false;
            for (i2=1;i2<=kol;i2++)
                if (mas_char[i2]==s.charAt(i)) {
                    razr=true;
                    break;
                }
            if (razr==false) {
                kol++;
                mas_char[kol]=s.charAt(i);
                mas_s[kol]=1;
            } else mas_s[i2]++;
        }
        while (true) {
            razr=false;
            for (int i = 1; i < kol; i++)
                if (mas_s[i] < mas_s[i + 1]) {
                    mas_s[0] = mas_s[i];
                    mas_s[i] = mas_s[i + 1];
                    mas_s[i + 1] = mas_s[0];
                    mas_char[0] = mas_char[i];
                    mas_char[i] = mas_char[i + 1];
                    mas_char[i + 1] = mas_char[0];
                    razr=true;
                }
            if (razr==false) break;
        }
